...
def fillH(double complex [:,:,:]temp_ham, double [:]u0list, double [:]uxlist, double [:]uylist,double [:]uzlist, double [:]muxlist, double [:]muylist, double [:]muzlist,
        int nbonds, int[:] ml,int[:]nl, double complex [:,:] phaselist,
          int nsites, int nband, double [:] jlist):
    ...
    for point in range(nsites): #sum over momenta
        ...
        for i in range(nbonds):
            ...
            uy = uylist[i]
            uz = uzlist[i]
            temp_ham[point, m1, n2] = temp_ham[point, m1, n2] - jlist[i]*(ux-uy*1j) * phaselist[i, point]
            temp_ham[point, m2, n1] = temp_ham[point, m2, n1] - jlist[i]*(ux+uy*1j) * phaselist[i, point]
                                                                         
            temp_ham[point, n2, m1] = temp_ham[point, n2, m1] - jlist[i]*(ux+uy*1j) * phaselist[i, point].conjugate()
            temp_ham[point, n1, m2] = temp_ham[point, n1, m2] - jlist[i]*(ux-uy*1j) * phaselist[i, point].conjugate()
                                                                         
            temp_ham[point, m1, n1] = temp_ham[point, m1, n1] - jlist[i]* (1j*u0+uz) * phaselist[i,point]
            temp_ham[point, m2, n2] = temp_ham[point, m2, n2] - jlist[i]*(1j*u0-uz)  *phaselist[i,point]
                                                                         
            temp_ham[point, n1, m1] = temp_ham[point, n1, m1] - jlist[i]* (-1j*u0+uz) * phaselist[i,point].conjugate()
            temp_ham[point, n2, m2] = temp_ham[point, n2, m2] - jlist[i]*(-1j*u0-uz)  *phaselist[i,point].conjugate()
...
def fillHkx(double complex [:,:,:]temp_ham, double [:]u0list, double [:]uxlist, double [:]uylist,double [:]uzlist, double [:]muxlist, double [:]muylist, double [:]muzlist,
        int nbonds, int[:] ml,int[:]nl, double complex [:,:] phaselist,
          int nsites, int nband, double [:] jlist):
    cdef int norbital = nband // 2
    cdef int point
